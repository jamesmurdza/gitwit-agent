...
export class Build {
    ...
    private constructor(props: BuildConstructorProps) {
    ...
    // Factory class that only returns a working object if Docker is found
    static async create(props: BuildConstructorProps): Promise<Build> {
        const classInstance = new Build(props);
        try {
            classInstance.dockerDaemonPath = await executeCommand("lsof -U -n -c docker | awk '/docker\\.sock/ {print $8}' | head -n 1");
        } catch (error: any) {
            console.log(`Unable to find a running Docker daemon.`)
            throw new Error(`Unable to find a running Docker daemon.`)
        }
    ...
    buildAndPush = async ({ debug = false, onStatusUpdate = async ({ }) => { }, } = {}) => {
        ...
        if (this.isBranch) {
            ...
            const sourceHTMLRoot = this.sourceGitURL?.replace(".git", ""); // What is this sourceHTML?
        ...
        // Define the parameters used by the scripts.
            ...
            const docker = new Docker({
                socketPath: this.dockerDaemonPath!,
